...
| `overlays[].labelSide` | enum | Which side of the line the label sits: `top` (default) \| `middle` \| `bottom`. On a line **steeper than 45° on screen** the clearing direction flips to horizontal — a few px of vertical nudge cannot clear a line that climbs further than that across the width of the text, so the label moves beside the line instead, on whichever side of the frame has more room, with its text running away from the line. `labelSide` then reads as near/far rather than above/below. Steepness is a SCREEN property, so it depends on the frame's aspect and the axis ranges, not on the data slope alone. |
