 ...
 * needed and the test runs in node environment without Playwright.
 */
...
/**
 * Create a solid-color PNG buffer with the given dimensions and RGBA fill.
 */
function makePng(
  width: number,
  height: number,
...
describe("comparePng — identical images", () => {
  it("returns match:true and diffPixels:0 for two identical 4x4 buffers", () => {
    const buf = makePng(4, 4, 100, 150, 200);
    const result = comparePng(buf, buf);
    expect(result.match).toBe(true);
    expect(result.diffPixels).toBe(0);
    expect(result.totalPixels).toBe(16);
  });

  it("returns match:true for two separately constructed identical buffers", () => {
    const buf1 = makePng(8, 8, 255, 0, 0);
    const buf2 = makePng(8, 8, 255, 0, 0);
    const result = comparePng(buf1, buf2);
    expect(result.match).toBe(true);
    expect(result.diffPixels).toBe(0);
  });
});
...
describe("comparePng — differing images", () => {
  it("returns match:false and diffPixels > 0 when images differ", () => {
  ...
  it("diffPixels equals totalPixels when every pixel differs and threshold is low", () => {
    const buf1 = makePng(2, 2, 0, 0, 0);
    const buf2 = makePng(2, 2, 255, 255, 255);
    const result = comparePng(buf1, buf2, { threshold: 0 });
    expect(result.match).toBe(false);
...
describe("comparePng — mismatched dimensions", () => {
  it("returns match:false with diffPixels = totalPixels (actual size) when dims differ", () => {
    const buf1 = makePng(4, 4, 100, 100, 100);
    const buf2 = makePng(8, 8, 100, 100, 100);
    const result = comparePng(buf1, buf2);
    expect(result.match).toBe(false);
    // totalPixels is based on actual (first) image
