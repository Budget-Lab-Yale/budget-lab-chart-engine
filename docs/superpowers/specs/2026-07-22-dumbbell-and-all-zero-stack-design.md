...
**Column mapping decision.** The dumbbell's spec uses `columns.category` / `.series` / `.value` and `category_order`. In this engine the categorical axis already *is* the resolved `x` field (how `bar`/`dotplot`/`waterfall` read their category axis), resolved once in `src/spec/columns.ts` — the single source of truth for the whole pipeline. **Decision: alias, not a new role.** `resolveColumns` accepts `columns.category` as a synonym for `columns.x` (`c.category ?? c.x ?? "time"`), and `category_order` as a synonym for `x_order`; `value` and `series` are unchanged. The dumbbell then reuses every path that keys off the resolved `x` (band scale, `fx`/`fy` faceting, horizontal left-gutter, tooltip category header, `category_colors`) with zero data-layer changes. The alternative — a first-class `category` role — would fork "categorical axis = `category` for dumbbell, `x` otherwise" through the mark/validation/tooltip code while mapping to the same band scale, buying nothing functional; it only moves the (unavoidable, tool-chosen) naming inconsistency from two lines of config resolution into threaded code. Aliasing keeps it contained.
